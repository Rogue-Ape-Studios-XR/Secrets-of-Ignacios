...
are made with 2 hands. To solve this issue a gesture system needed to be created That can recognise 2 handed gestures. For this reason the [`HandShape` enum](../../Assets/Scripts/Gestures/HandShape.cs), [`Gesture` scriptable object](../../Assets/Scripts/Gestures/Gesture.cs) and [`SequenceChecker` script](../../Assets/Scripts/Gestures/SequenceChecker.cs) were created.

|Gesture Diagram|
|-|
|![afbeelding](https://github.com/user-attachments/assets/92979d93-2665-418c-8e17-3610104da5e3)|
...
- `HandleOnSpellValidated()`:
    Clears validated gestures and resets the sequence when a spell is successfully cast.
- `HandleOnSpellFailed()`:
    Clears gestures, resets the sequence, and invokes the event to disable VFX on hands when no spell is matched.
- `CheckGestures()`:  
